...
export class PosixService {
  ...
  public getDefaultFrequenceWithDefault(): CronJobsFrequency {
    ...
    cronJobsFrequency.daysOfWeek = this.getDaysOfWeek()[0]
      ? [this.getDaysOfWeek()[0].value]
      : [];
    cronJobsFrequency.daysOfMonth = this.dataService.daysOfMonth[0]
      ? [this.dataService.daysOfMonth[0].value]
      : [];
    cronJobsFrequency.months = this.dataService.months[0]
      ? [this.dataService.months[0].value]
      : [];
    cronJobsFrequency.hours = this.dataService.hours[0]
      ? [this.dataService.hours[0].value]
      : [];
    cronJobsFrequency.minutes = this.dataService.minutes[0]
      ? [this.dataService.minutes[0].value]
      : [];
    cronJobsFrequency.nthMinutes = this.dataService.minutes[0]
      ? [this.dataService.minutes[0].value]
      : [];
    return cronJobsFrequency;
  }
  ...
  private fromCronInternal(
    ...
    if (cron[0] !== "*") {
      ...
      if (cron[0].includes("*/")) {
        frequency.baseFrequency = this.baseFrequency.minute;
        frequency.nthMinutes = this.getValueArray(cron[0]);
      } else {
    ...
    if (cron[1] !== "*") {
      ...
      console.log(this.getValueArray(cron[1]));
  ...
  setCron(value: CronJobsFrequency) {
    ...
    if (value && value.baseFrequency) {
      ...
      if (value.baseFrequency === this.baseFrequency.week) {
        cron[4] =
          value.daysOfWeek.length > 0 ? value.daysOfWeek.join(",") : "*";
      }

      if (value.baseFrequency >= this.baseFrequency.month) {
        cron[2] =
          value.daysOfMonth.length > 0 ? value.daysOfMonth.join(",") : "*";
      }
      ...
      if (
        value.baseFrequency === this.baseFrequency.minute &&
        value.nthMinutes.length &&
        value.nthMinutes[0] !== 0
      ) {
