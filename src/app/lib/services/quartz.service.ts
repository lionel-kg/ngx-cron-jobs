  ...
  setCron(newValue: CronJobsFrequency) {
    ...
    if (newValue && newValue.baseFrequency) {
      if (newValue.baseFrequency >= this.baseFrequency.hour) {
        cron[1] =
          newValue.minutes.length > 0 ? newValue.minutes.join(",") : "*";
      }
      ...
      if (newValue.baseFrequency === this.baseFrequency.week) {
        cron[3] = "?";
        cron[5] =
          newValue.daysOfWeek.length > 0 ? newValue.daysOfWeek.join(",") : "*";
      }

      if (newValue.baseFrequency >= this.baseFrequency.month) {
        cron[3] =
          newValue.daysOfMonth.length > 0
            ? newValue.daysOfMonth.join(",")
            : "*";
      }
      ...
      if (
        newValue.baseFrequency === this.baseFrequency.minute &&
        newValue.nthMinutes.length &&
        newValue.nthMinutes[0] !== 0
      ) {
